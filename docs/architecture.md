...
- Appointment client/therapist references, time range, status, and timestamps.
...
4. Creates a REQUESTED appointment and an opaque managed Google event in one coordinated workflow; it removes the D1 appointment if event publication fails.

Staff can create confirmed appointments and busy blocks, approve pending appointments, move managed items, or cancel them. Google events are generic managed entries and do not contain client names, email addresses, services, notes, or clinical content. Calendar changes received through a validated webhook or scheduled synchronization update the matching D1 metadata and appointment state where appropriate.
...
| POST /api/appointments/book | Requires a verified USER session; accepts a strict JSON object containing an ISO timestamp with offset in startsAt and an optional UUID rescheduleFromAppointmentId. Returns an appointment identifier or a safe validation/conflict response. |
