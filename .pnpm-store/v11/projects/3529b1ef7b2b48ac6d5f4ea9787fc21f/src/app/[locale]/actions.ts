"use server"
// Role changes are privilege changes. Force a fresh authentication and
// second-factor check before the target can use the new role.
...
export async function createAvailability(locale: Locale, formData: FormData) {
  ...
  const requestedTherapistId = String(
    formData.get("therapistId") ??
      "",
  )
  if (
    actor.role ===
    ...
    throw new Error("Select a therapist")
  const therapistId =
    actor.role ===
      "ADMIN" &&
    requestedTherapistId
  ...
  await createAvailabilitySlot(
    therapistId,
    formDate(formData.get("startsAt")),
    formDate(formData.get("endsAt")),
  )
...
export async function blockAvailability(locale: Locale, slotId: string) {
  ...
  if (
    ...
    throw new Error("Only open availability can be blocked")
  await db
  ...
  if (
    ...
    if (appointment.clientId) {
      ...
      if (client?.emailVerified) {
        const message = appointmentEmail(
          status ===
            "CONFIRMED"
            ? "confirmed"
            : "cancelled",
...
export async function changeStaffRole(
  ...
  if (
    ...
    throw new Error("Use a second administrator for role changes")
  if (!["USER", "THERAPIST", "ADMIN"].includes(nextRole))
  ...
  if (
    isStaff(nextRole) &&
    (!target.emailVerified || !target.twoFactorEnabled)
  ) {
    throw new Error(
  ...
  defer(
    sendOperationsEmail(
      getRuntimeEnv(),
      "Eikon Mind staff role updated",
      "A staff role was changed. Review the administration audit and access procedure.",
    ),
  )
  revalidatePath(`/${locale}/admin`)
}
...
export async function requestAccountDeletion(locale: Locale) {
  ...
  await db.batch([
    ...
    db
      ...
      .values({ id: crypto.randomUUID(), userId: user.id, requestedAt: now }),
  ])
