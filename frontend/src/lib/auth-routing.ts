...
export function authDestination(locale: string, role: Role, requestedPath?: string | null) {
  const section = role === "USER" ? "client" : "admin";
  if (isSafeRelativePath(requestedPath, locale, section)) return requestedPath as string;
