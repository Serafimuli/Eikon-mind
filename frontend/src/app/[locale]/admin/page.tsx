...
export default async function StaffDashboard({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const [upcoming] = await (scope
    ? db
        .select({ count: count() })
        .from(appointments)
        .where(and(eq(appointments.therapistId, user.id), gte(appointments.startsAt, new Date())))
    : db
        .select({ count: count() })
        .from(appointments)
    ...
    <main className="private-shell">
      <div className="private-head">
        <div>
          ...
          <h1>{user.role === "ADMIN" ? "Administration" : "Therapist workspace"}</h1>
        </div>
      ...
      <div className="card-grid">
        ...
        <div className="card">
          <h3>Actions</h3>
          <Link href={`/${locale}/admin/appointments`}>View appointments</Link>
          {user.role === "ADMIN" && (
            <>
              <br />
              <Link href={`/${locale}/admin/staff`}>Manage staff</Link>
            </>
          )}
        </div>
      </div>
