...
export function EmailVerificationCard({
  ...
  if (verified)
    return (
      <p className="success" role="status">
        {locale === "ro" ? "Adresa de email este verificată." : "Email address verified."}
      </p>
    );
  ...
  const resend = async () => {
    ...
    if (result.error) {
      captcha.reset();
      return setError(
        locale === "ro" ? "Nu am putut retrimite emailul." : "We could not resend that email.",
      );
    }
    setMessage(
      locale === "ro"
  ...
  return (
    <section className="card profile-card profile-card--security" aria-live="polite">
      ...
      <button
        ...
        type="button"
        disabled={busy || !captcha.token}
        onClick={resend}
      >
        {busy ? "…" : locale === "ro" ? "Retrimite emailul" : "Resend verification email"}
      </button>
    </section>
  );
}
