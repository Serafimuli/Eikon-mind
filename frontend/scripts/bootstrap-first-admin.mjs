...
if (!configPath || !/^[0-9a-f-]{36}$/i.test(userId ?? "")) {
  throw new Error("Usage: node scripts/bootstrap-first-admin.mjs <generated-wrangler-config> <verified-totp-user-uuid>");
}

const config = JSON.parse(readFileSync(configPath, "utf8"));
const databaseName = config.d1_databases?.[0]?.database_name;
if (!databaseName) throw new Error("Could not read D1 database_name from the generated Wrangler configuration");
...
const result = spawnSync(executable, ["wrangler", "d1", "execute", databaseName, "--remote", "--config", configPath, "--command", command], {
  stdio: "inherit",
  shell: false,
});
